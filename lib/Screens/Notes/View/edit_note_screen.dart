...
class _EditNotesScreenState extends State<EditNotesScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      notesController.isEdit.value = false;
      notesController.notesNameList.clear();
      if((notesController.selectedNote.value.date??"").isNotEmpty){
        DateFormat format = DateFormat("dd-MM-yyyy");
        selectedDate = format.parse(notesController.selectedNote.value.date??"");

        setState(() {

        });
      }
      if ((notesController.selectedNote.value.note ?? []).isNotEmpty) {
        for (int i = 0;
        i < (notesController.selectedNote.value.note ?? []).length;
        i++) {
          NoteNameData addNoteNameModel = NoteNameData();
          addNoteNameModel.id = notesController
              .selectedNote.value.note?[i].id
              .toString();
          addNoteNameModel.textEditingController.text = notesController
              .selectedNote.value.note?[i].name ??
              "";

          notesController.notesNameList.add(addNoteNameModel);
        }
      } else {
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () {
              notesController.isEdit.value = true;
            },
          ),
          ...
          IconButton(
            ...
            onPressed: () {
              Get.defaultDialog(
                  ...
                  onConfirm: () async {
                    Navigator.pop(context);
                    notesController.callDeleteNote(notesController.selectedNote.value.id.toString());
          ...
          SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                // Login Button
                notesController.isEdit.value?Padding(
                  ...
                  child: CommonButton(
                    ...
                    if(notesController.notesNameList.isEmpty){
                      ...
                      notesController.createNotesRequest.value.id = notesController.selectedNote.value.id.toString();
                      ...
                      for(int i=0; i<notesController.notesNameList.length; i++){
                        ...
                        child.id =
                            notesController.notesNameList[i].id ?? "";
